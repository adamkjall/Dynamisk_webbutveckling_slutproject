...
const paymentInfo = (order) => {
  return (
    ...
    >
      ...
      <Text>{`Additional fee: ${getPaymentFee(
        order.paymentMethod.price
      )}`}</Text>
    </Box>
  );
};
...
const productSection = (order) => {
  return (
    <Box key={`productSection-${order._id}`} pad="small" background="light-2">
    ...
    <>
      {adminControls && updateStatus && (
        <StyledDeliveryButton
          margin="small"
          primary
          label={order.orderStatus ? "Undo delivery" : "Mark as delivered"}
          color=""
